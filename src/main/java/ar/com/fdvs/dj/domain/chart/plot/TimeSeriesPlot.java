...
public class TimeSeriesPlot extends AbstractPlot {
	...
	public void transform(DynamicJasperDesign design, JRChartPlot plot, String name) {
		...
		final JRDesignTimeSeriesPlot timeSeriesPlot = (JRDesignTimeSeriesPlot) plot;

		final StringExpression timeAxisLabelExp = getTimeAxisFormat().getLabelExpression();
		if (timeAxisLabelExp != null) {
			final JRDesignExpression exp = ExpressionUtils.createAndRegisterExpression(design, "timeAxisLabel_" + name, timeAxisLabelExp);
		...
		final CustomExpression timeAxisRangeMinValueExp = getTimeAxisFormat().getRangeMinValueExpression();
		if (timeAxisRangeMinValueExp != null) {
			final JRDesignExpression exp = ExpressionUtils.createAndRegisterExpression(design, "timeAxisRangeMinValue_" + name, timeAxisRangeMinValueExp);
			timeSeriesPlot.setDomainAxisMinValueExpression(exp);
		}
		final CustomExpression timeAxisRangeMaxValueExp = getTimeAxisFormat().getRangeMaxValueExpression();
		if (timeAxisRangeMaxValueExp != null) {
			final JRDesignExpression exp = ExpressionUtils.createAndRegisterExpression(design, "timeAxisRangeMaxValue_" + name, timeAxisRangeMaxValueExp);
