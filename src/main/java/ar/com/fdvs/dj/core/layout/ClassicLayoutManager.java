...
public class ClassicLayoutManager extends AbstractLayoutManager {
	...
	protected void applyHeaderAutotexts() {
		...
		final ArrayList<HorizontalBandAlignment> positions = new ArrayList<>();
		...
		final int totalYoffset = findTotalOffset(positions, AutoText.POSITION_HEADER);
		LayoutUtils.moveBandsElemnts(totalYoffset, headerband);

		for (final HorizontalBandAlignment currentAlignment : positions) {
			int yOffset = 0;

			for (final AutoText text : getReport().getAutoTexts()) {
	...
	protected int findTotalOffset(ArrayList aligments, byte position) {
		int total = 0;
		for (final Object aligment : aligments) {
			final HorizontalBandAlignment currentAlignment = (HorizontalBandAlignment) aligment;
			int aux = 0;
			for (final AutoText autotext : getReport().getAutoTexts()) {
    ...
    protected List<AbstractColumn> getVisibleColumns() {
		final List<AbstractColumn> visibleColums = new ArrayList<>(getReport().getColumns());
	...
	protected void applyBanners() {
        final DynamicReportOptions options = getReport().getOptions();
	...
	protected void applyImageBannersToBand(JRDesignBand band, Collection imageBanners, JRDesignExpression printWhenExpression, boolean placeOnTop ) {
		int maxHeight = 0;
        if (placeOnTop){
			for (final Object imageBanner1 : imageBanners) {
				final ImageBanner imageBanner = (ImageBanner) imageBanner1;
		...
		if (band != null){
			...
			for (final Object imageBanner1 : imageBanners) {
				final ImageBanner imageBanner = (ImageBanner) imageBanner1;
				final String path = "\"" + imageBanner.getImagePath().replace('\\', '/') + "\"";
				...
				if (imageBanner.getAlign() == ImageBanner.Alignment.Left) {
					...
					final int x = (getReport().getOptions().getPage().getWidth() -
	...
	protected void generateTitleBand() {
		...
		final JRDesignExpression printWhenExpression = new JRDesignExpression();
	...
	protected void layoutGroups() {
		...
		for (final DJGroup columnsGroup : getReport().getColumnsGroups()) {
			final JRDesignGroup jgroup = getJRGroupFromDJGroup(columnsGroup);
			...
			final JRDesignSection headerSection = (JRDesignSection) jgroup.getGroupHeaderSection();
			final JRDesignSection footerSection = (JRDesignSection) jgroup.getGroupFooterSection();
			...
			if (columnsGroup.getLayout().isPrintHeaders()) {
				...
				final int groupIdx = getReport().getColumnsGroups().indexOf(columnsGroup);
				if (groupIdx > 0) {
					final DJGroup prevG = getReport().getColumnsGroups().get(groupIdx - 1);
					if (!(prevG.getColumnToGroupBy() instanceof GlobalGroupColumn)) {
                        skipPreviousGroupHeaders = !prevG.getLayout().isShowValueForEachRow();
                    }
				}
				for (final Object o : getVisibleColumns()) {
					...
					final JRDesignTextField designTextField = createColumnNameTextField(columnsGroup, col);
			...
			final DJGroupLabel label = columnsGroup.getFooterLabel();
			if (label != null) {
				final List<DJGroupVariable> footerVariables = columnsGroup.getFooterVariables();
				final PropertyColumn col = columnsGroup.getColumnToGroupBy();
				int x = 0, y = 0;
				//max width
				...
				if (label.getLabelPosition() == LabelPosition.LEFT) {
					final DJGroupVariable lmvar = findLeftMostColumn(footerVariables);

					x = col.getPosX(); //label starts in the column-to-group-by x position
					y = findYOffsetForGroupLabel(footer);
					if (lmvar != null) {
						final AbstractColumn lmColumn = lmvar.getColumnToApplyOperation();
					...
					final DJGroupVariable rmvar = findRightMostColumn(footerVariables);

					if (rmvar != null) {
						final AbstractColumn rmColumn = rmvar.getColumnToApplyOperation();
	...
	protected void layoutGroupFooterLabels(DJGroup djgroup, JRDesignGroup jgroup, int x, int y, int width, int height) {
		...
		final DJGroupLabel label = djgroup.getFooterLabel();

		//if (label == null || footerVariables.isEmpty())
			//return;

		//PropertyColumn col = djgroup.getColumnToGroupBy();
		final JRDesignBand band =  LayoutUtils.getBandFromSection((JRDesignSection) jgroup.getGroupFooterSection());
...
//		log.debug("Adding footer group label for group " + djgroup);
		...
		final JRDesignTextField labelTf = new JRDesignTextField();
	...
	protected void layoutGroupCrosstabs(DJGroup columnsGroup,	JRDesignGroup jgroup) {
		for (final DJCrosstab djcross : columnsGroup.getHeaderCrosstabs()) {
			final Dj2JrCrosstabBuilder djcb = new Dj2JrCrosstabBuilder();

			final JRDesignCrosstab crosst = djcb.createCrosstab(djcross, this);
			final JRDesignBand band = LayoutUtils.getBandFromSection((JRDesignSection) jgroup.getGroupHeaderSection());
			if (djcross.getBottomSpace() != 0) {
				final JRDesignRectangle rect = createBlankRectableCrosstab(djcross.getBottomSpace(), 0);
			...
			final DJLabel caption = djcross.getCaption();
			if (caption != null) {
				...
				if (caption.isJasperExpression()) { //a text with things like "$F{myField}"
					...
					final String name = "expression_for_label_at_header_of_group[" + getReport().getColumnsGroups().indexOf(columnsGroup) + "]_crosstab[" + columnsGroup.getHeaderCrosstabs().indexOf(djcross) + "]";
					LayoutUtils.registerCustomExpressionParameter((DynamicJasperDesign) getDesign(), name, caption.getLabelExpression());
					final String invocationText = ExpressionUtils.createCustomExpressionInvocationText(caption.getLabelExpression(), name, false);
				...
				final JRDesignTextField captTf = new JRDesignTextField();
		...
		for (final DJCrosstab djcross : columnsGroup.getFooterCrosstabs()) {
			final Dj2JrCrosstabBuilder djcb = new Dj2JrCrosstabBuilder();

			final JRDesignCrosstab crosst = djcb.createCrosstab(djcross, this);
			final JRDesignBand band = LayoutUtils.getBandFromSection((JRDesignSection) jgroup.getGroupFooterSection());
			final int yOffset = LayoutUtils.findVerticalOffset(band);
			if (djcross.getTopSpace() != 0) {
...
//		rect.setBackcolor(Color.RED);
	...
	protected JRDesignTextField createColumnNameTextField(DJGroup columnsGroup, AbstractColumn col) {
		final JRDesignTextField designStaticText = new JRDesignTextField();
		final JRDesignExpression exp = new JRDesignExpression();
	...
	protected void layoutGroupSubreports(DJGroup columnsGroup, JRDesignGroup jgroup) {
		...
		final JRDesignBand footerBand = (JRDesignBand) ((JRDesignSection)jgroup.getGroupFooterSection()).getBandsList().get(0);
		final JRDesignBand headerBand = (JRDesignBand) ((JRDesignSection)jgroup.getGroupHeaderSection()).getBandsList().get(0);
	...
	protected void layOutSubReportInBand(DJGroup columnsGroup, JRDesignBand band, String position) {
		...
		final List<Subreport> subreportsList = DJConstants.FOOTER.equals(position)
				? columnsGroup.getFooterSubreports()
				: columnsGroup.getHeaderSubreports();

		for (final Subreport sr : subreportsList) {
			final JRDesignSubreport subreport = new JRDesignSubreport(new JRDesignStyle().getDefaultStyleProvider());

			//The data source
			final int dataSourceOrigin = sr.getDatasource().getDataSourceOrigin();
			if (DJConstants.DATA_SOURCE_ORIGIN_USE_REPORT_CONNECTION == dataSourceOrigin) {
				final JRDesignExpression connectionExpression = ExpressionUtils.getReportConnectionExpression();
				...
				final JRDesignExpression connectionExpression = ExpressionUtils.getConnectionExpression(sr.getDatasource());
				subreport.setConnectionExpression(connectionExpression);
			} else {
				final JRDesignExpression dataSourceExpression = ExpressionUtils.getDataSourceExpression(sr.getDatasource());
				subreport.setDataSourceExpression(dataSourceExpression);
			}

			final String paramname = sr.getName(); //TODO ensure this name is unique among all possible subreports
			((DynamicJasperDesign) getDesign()).getParametersWithValues().put(paramname, sr.getReport());
			final String expText = "(" + JasperReport.class.getName() + ")$P{REPORT_PARAMETERS_MAP}.get( \"" + paramname + "\" )";
			final JRDesignExpression srExpression = ExpressionUtils.createExpression(expText, JasperReport.class);
			subreport.setExpression(srExpression);


			//set the parameters
			subreport.setParametersMapExpression(ExpressionUtils.getParameterExpression(sr));
			for (final SubreportParameter srparam : sr.getParameters()) {
				...
				final JRExpression expression2 = ExpressionUtils.createExpression(getDesign(), srparam);
			...
			if (sr.isStartInNewPage()) {
				final JRDesignGroup jrgroup = getJRGroupFromDJGroup(columnsGroup);
...
//						targetBand = (JRDesignBand) ((JRDesignGroup) getDesign().getGroupsList().get(idx-1)).getGroupHeader();
					...
					final JRDesignGroup jrGroup = (JRDesignGroup) getDesign().getGroupsList().get(idx);
				...
				if (DJConstants.FOOTER.equals(position) && (targetBand != null)) {
					final JRDesignBreak pageBreak = new JRDesignBreak(new JRDesignStyle().getDefaultStyleProvider());
	...
	protected void sendPageBreakToBottom(JRDesignBand band) {
		final JRElement[] elems = band.getElements();
		JRElement aux = null;
		for (final JRElement elem : elems) {
...
//			jgroup.setGroupFooter(footerBand);
		...
		int yOffset = 0;
		final GroupLayout layout = group.getLayout();
		//Only the value in header
		final PropertyColumn column = group.getColumnToGroupBy();

		final int height = group.getHeaderVariablesHeight()!= DynamicReportOptions.UNSET_VALUE
		...
		if (layout.isShowValueInHeader() && layout.isHideColumn() && !layout.isShowColumnName()){
			...
			final JRDesignTextField currentValue = generateTextFieldFromColumn(column, height, group);
            ...
            if (!group.getHeaderVariables().isEmpty()){
            	final DJGroupVariable leftmostcol = findLeftMostColumn(group.getHeaderVariables());
...
//			currentValue.setHeight(FontHelper.getHeightFor(column.getStyle().getFont())); //XXX CAREFULL
		...
		//VALUE_IN_HEADER_WITH_HEADERS_AND_COLUMN_NAME
			...
			final JRDesignTextField columnNameTf = createColumnNameTextField(group, column);
	...
	protected boolean groupLabelsPresent(List groupVariables) {
		for (final Object groupVariable : groupVariables) {
			final DJGroupVariable var = (DJGroupVariable) groupVariable;
	...
	protected void placeVariableInBand(List<DJGroupVariable> variables, DJGroup djGroup, JRDesignGroup jgroup, String type, JRDesignBand band, int yOffset) {
		...
		final boolean inFooter = DJConstants.FOOTER.equals(type);
		...
		int yOffsetGlabel = 0;
		for (final DJGroupVariable var : variables) {
			final AbstractColumn col = var.getColumnToApplyOperation();

			final String variableName = var.getName();

			//Add the group label
			final DJGroupLabel label = var.getLabel();
			...
			final Style defStyle = DJConstants.HEADER.equals(type)
			...
			if (var.getPrintWhenExpression() != null) {
				final JRDesignExpression exp = ExpressionUtils.createExpression(variableName + "_printWhenExpression", var.getPrintWhenExpression(), true);
		...
		if (djGroup.getColumnToGroupBy() instanceof GlobalGroupColumn) {
			...
			final DJGroupVariable leftmostColumn = findLeftMostColumn(variables);
			totalWidth = leftmostColumn.getColumnToApplyOperation().getPosX();

			final GlobalGroupColumn globalCol = (GlobalGroupColumn) djGroup.getColumnToGroupBy();

			final JRDesignTextField globalTextField = new JRDesignTextField();
			final JRDesignExpression globalExp = new JRDesignExpression();
	...
	protected DJGroupVariable findLeftMostColumn(List<DJGroupVariable> variables) {
		...
		for (final DJGroupVariable groupVariable : variables) {
	...
	protected DJGroupVariable findRightMostColumn(List variables) {
		...
		for (final Object variable : variables) {
			final DJGroupVariable currentCol = (DJGroupVariable) variable;
	...
	protected void insertValueInHeader(JRDesignBand headerBand, DJGroup djgroup, int headerOffset) {
		...
		final JRDesignTextField textField = generateTextFieldFromColumn(djgroup.getColumnToGroupBy(), height, djgroup);
	...
	private void setUpAnchorNameForGroupValue(DJGroup djgroup, JRDesignTextField textField) {
		final int groupIdx = getReport().getColumnsGroups().indexOf(djgroup);
		textField.setBookmarkLevel(groupIdx);

		final JRDesignExpression jrGroupExpression = (JRDesignExpression) textField.getExpression();
		//Anchors are hardcoded as String in JR, so we need to make sure the expression will be rendered as a string
		final String text = "\"\" +" + jrGroupExpression.getText();
		final JRDesignExpression expressionForAnchor = new JRDesignExpression(text);
	...
	protected int changeHeaderBandHeightForVariables(JRDesignBand headerBand, DJGroup columnsGroup) {
		...
		if (!headerBand.getChildren().isEmpty()) {
			final int acu = headerBand.getHeight();
...
//		log.debug("transforming detail band text field...");
		final DJGroup group = getDJGroup(column);
